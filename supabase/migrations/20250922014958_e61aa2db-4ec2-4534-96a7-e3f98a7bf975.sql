...
DECLARE
    ...
    current_timestamp TIMESTAMPTZ := '2025-09-01 00:00:00+08'::timestamptz;
    counter INTEGER := 1;
BEGIN
    -- Clear existing data
    ...
    WHILE current_timestamp <= end_date LOOP
        INSERT INTO public.processed_sensor_readings (
            ...
            25.0 + 5.0 * sin(EXTRACT(HOUR FROM current_timestamp) * PI() / 12) + (random() - 0.5) * 3.0,
            
            -- Humidity: 40-80% with inverse temperature correlation
            65.0 - 15.0 * sin(EXTRACT(HOUR FROM current_timestamp) * PI() / 12) + (random() - 0.5) * 10.0,
            
            -- Pressure: 1000-1020 hPa with weather patterns
            1013.25 + 5.0 * sin(EXTRACT(DOY FROM current_timestamp) * PI() / 15) + (random() - 0.5) * 3.0,
