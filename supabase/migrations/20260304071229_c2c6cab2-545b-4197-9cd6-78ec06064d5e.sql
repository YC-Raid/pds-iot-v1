-- Prevent timeouts and sync stampedes by batching sync and using an advisory lock
...
BEGIN
  -- Only admin users can sync data (defense in depth)
  IF NOT public.has_role(auth.uid(), 'admin'::public.app_role) THEN
    RAISE EXCEPTION 'Admin access required';
  END IF;

  -- Prevent concurrent sync storms from repeated frontend calls
  SELECT pg_try_advisory_xact_lock(982451653) INTO got_lock;
  IF NOT got_lock THEN
    RETURN 0;
  END IF;

  SELECT COALESCE(MAX(original_id), 0)
  INTO max_original_id
