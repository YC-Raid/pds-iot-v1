-- Fix sync function: remove references to columns that don't exist in sensor_data
...
BEGIN
  ...
  -- Note: sensor_data does NOT have door_status, door_opens, intrusion_alert columns
  ...
  SELECT 
    ...
    SQRT(POWER(COALESCE(sd.accel_x, 0), 2) + POWER(COALESCE(sd.accel_y, 0), 2) + POWER(COALESCE(sd.accel_z, 0), 2)),
    -- Calculate gyro magnitude
    SQRT(POWER(COALESCE(sd.gyro_x, 0), 2) + POWER(COALESCE(sd.gyro_y, 0), 2) + POWER(COALESCE(sd.gyro_z, 0), 2)),
    -- Default values for columns that don't exist in source
    'CLOSED',
    0,
    FALSE,
    'hangar_01'
